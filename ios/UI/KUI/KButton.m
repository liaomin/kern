...
- (CGSize)measure:(CGFloat)width widthMode:(MeasureMode)widthMode height:(CGFloat)height heightMode:(MeasureMode)heightMode{
   if(widthMode == MeasureModeExactly && heightMode == MeasureModeExactly){
    ...
    CGRect rect =  [label.text boundingRectWithSize:CGSizeMake(maxWidth, maxHeight) options:NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attrubutes context:nil];
...
- (void)setPadding:(UIEdgeInsets)padding
{
    ...
    [self setTitleEdgeInsets:padding];
}
